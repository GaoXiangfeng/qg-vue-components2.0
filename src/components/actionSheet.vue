...
<template>
    <div class="action-sheet" :class="{'multiterm': multiterm,'banktiterm': isScroll }" v-show="show" @click="close" qg-log="关闭ActionSheet">
        <div class="action-sheet">
            <div @click.stop class="action-sheet-wrap qg-vbox" :class="{'slideInUp':showAnimate,'action-sheet-wrap185':height185}">
                <slot name="title">
                    <div class="title">{{title}}</div>
                </slot>
                <slot name="content">
                    <div v-if="multiterm" class="nav">
                        <label v-for="(item, index) in navTitle" :key="item.title" :class="{selected: currentNav==index}" @click="navChange(index)">
                            {{item.title}}
                        </label>
                    </div>
                    <div class="group qg-flex-1">
                        <div>
                            <div @click="itemClick(index)" :class="{active:selectedIndex[currentNavPage]==index}" v-for="(item, index) in data" :key="item.value">
