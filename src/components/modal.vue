    ...
    <transition name="qg-zoom">
        <div class="qg-modal" v-show="open">
            ...
            <div class="qg-modal-footer" v-if="button" @touchmove="prevent($event)">
                <slot name="footer">
                    <div class="qg-box">
                        <div @click="handleCancel" class="qg-modal-footer-cancel qg-flex-1" v-if="button.length>1" qg-log="modal取消">
                            {{button[1].text}}
                        </div>
                        <div @click="handleConfirm" class="qg-modal-footer-confirm qg-flex-1" qg-log="modal确认">
                            {{button[0].text}}
                        </div>
                    </div>
                </slot>
...
export default {
    ...
    methods: {
        close() {
            store.dispatch('closeModal', this.name)
        },
        overlayClick() {
            this.close()
        },
